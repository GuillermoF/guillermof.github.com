...
function make_path(start, end, grid, astar_grid) {
	...
	var x = start[0] / 20; var y = start[1] / 20; var g = 0; 
	var best_move = [[astar_grid[x][y], g, x, y]];
	expanded[[x,y]] = 1;
	while (((x != goalx) || y != goaly)) {
		for (i = 0; i < moves.length; i++) {
			newx = x + moves[i][0];
			newy = y + moves[i][1];
			if (newx >= 0 && newy >= 0 && newx < canvas.width / 20 && newy < canvas.height / 20 && grid[newx][newy] == 0 && !([newx,newy] in expanded))
			{
				g = best_move[0][1] + 1;
				compare.push([astar_grid[newx][newy], g, newx, newy, i]);
			}
		}
		...
		best_move = compare.splice(0,1);
		x = best_move[0][2];y = best_move[0][3];
		actions[x][y] = best_move[0][4];
		expanded[[x, y]] = 1;
	}

	//now extract optimal path
	x = goalx; y = goaly;
	endx = start[0]/20; endy = start[0]/20;
	do {
		x -= moves[actions[x][y]][0];
		y -= moves[actions[x][y]][1];
		set_bgcolor(x*20, y*20, 'path');
	} while (x != endx || y != endy);

/* Dynamic Programming
 *
 * -> execution time too expensive
	change = true;
